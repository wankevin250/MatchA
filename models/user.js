...
const checkUser = (user) => {
  let isInvalid = true;
  if (user.username && user.username.length > 2 && user.username.length < 25 &&
    user.password && user.password.match(/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/) &&
    user.displayname && user.displayname.length > 2 && user.displayname < 25 &&
    user.firstname && user.firstname.match(/^[A-Z][a-z0-9_-]{1,25}$/) &&
    user.lastname && user.lastname.match(/^[A-Z][a-z0-9_-]{1,25}$/) &&
